<template>
	<view>
		<view class="proItem mb41" v-for="item in recData" @click="jumpPage('../coupon/couponBuyIndex?id='+item.id)">
			<view style="padding: 35rpx 20rpx;">
				<view class="flex">
					<view style="height: 200rpx;width: 200rpx; border-radius: 20rpx;overflow: hidden;" class="center">
						<nut-image width="100%" height="100%" :src="item.full_poster" style="border-radius: 20rpx;"
									showLoading>
						<template #loading>
					...
					<view class="mglr35" style="flex: 1;">
						...
						<view class="mt16 between">
							<view style="width: 240rpx; ">
								...
								<nut-progress style="height: 28rpx;240rpx"
									:percentage="item.jindu"
									:is-show-percentage="true" :text-inside="false"
									:text-color="store.$state.contentColor" :stroke-color="'red'" />
								<view class="f20 mt23">
		...
		setup(props, {
			...
			const getData = async (id) => {
				request({
					...
					recData.value.forEach(item=>{
						item.jindu = ((item.sale_panel>item.total_panel?item.total_panel:item.sale_panel) /item.total_panel * 100).toFixed(2)
					})
