<template>
	<view class="normalBg" style="background: #060D1F">
		<kefu></kefu>
		<topNav :title="t('add2.a_a2')"></topNav>
		<view class="pdlr35 ">
   <!-- 登录失效 -->
		   ...
		   <view class="">
			  <z-paging
				 ...
				 :empty-view-text="t('record.r_a1')"
				 :loading-more-no-more-text="t('record.r_a1')" :empty-view-reload-text="t('record.r_a2')" :loading-more-default-text="t('rec.r_a2')"
				 :loading-more-loading-text="t('record.r_a3')" :loading-more-fail-text="t('record.r_a4')" 
			  >
				 <view class="noticeItem" v-for="(item, index) in recordsList" :key="index" @click="readHandle(item)">
					...
					<view class="mglr32 f24">
					   <view class="textHiddenOne" style=" width: 500rpx;color: #000;">
