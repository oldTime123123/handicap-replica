		...
		<view class="pdlr30 mt48">
			...
			<div class="" v-if="tabsInd == 0" dir="ltr">
				...
				<view class="couponItem between" v-for="item in couponList">
					...
					<view  style="flex: 1;">
						<view class="f26"> {{t('newAct.c_l3')}} <text style="color: #E93337;">{{item.price}}: {{currency}}</text>
						</view>
						...
						<view class="mt30 recBtn" v-if="item.status == 2" @click="ReceiveHandle(item)">{{t('newAct.c_l5')}}</view>
