		...
		<view class="pdlr30 pt33 main">
			...
			<view class="box1">
				...
				<view class="text">
					...
					<view class="">
						{{detailData?.limit==0?'No limit on purchase quantity':detailData?.limit==-1?0:detailData?.limit}}({{t('ttn.t_t47')}}:
						{{ detailData?.buy_num }} )
					</view>
				</view>
				<view class="text">
				...
				<view class="text" v-show='detailData?.invite_type==1'>
					<view class="">
						{{t('ttn.t_t46')}}
					</view>
					<view class="" style="margin-top: 30rpx;">
				...
				<view class="text" v-show='detailData?.invite_type==2'>
					<view class="">
						{{t('ttn.t_t46')}}
					</view>
					<input type="text" v-model="amount" placeholder="please enter">
				</view>
			...
			<view class="box1">
				...
				<view class="text">
					...
					<view class="" style="font-size: 26rpx;text-align: right;">
						{{detailData?.min_num}}*{{number}}*{{(detailData?.rate*100).toFixed(2)}}%*{{detailData?.day}}
						day = {{t('ttn.t_t55')}}
						{{(Number(detailData?.min_num)*Number(detailData?.day)*Number(detailData?.rate)*Number(number).toFixed(2))+(Number(detailData?.min_num)*number)}}
						{{ currency }}
					</view>
...
<script setup>
	...
	const invest = () => {
		...
		if (detailData.value.buy_status == 0) {
			return Toast.text(
				'The maximum purchase limit for this product has been exceeded. Please change to another product and try again.'
			)
		}
