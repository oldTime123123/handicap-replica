<template>
	<view style="background: #060D1F">
		<view class="between" :style="{background:store.$state.contentColor}" style="height: 88rpx; padding: 0 31rpx;">
			...
			<view class="f36  text_white textHiddenOne center" style="font-weight: 500;width: 60%;height: 100%; ">{{t('newAct.r_r1')}}</view>
		...
		<view class=" mt40">
			...
			<view class="pdlr40">
				<view class="inputBox">
					...
					<view class="mt20 f26">
						{{t('newAct.r_r7')}} <text class="error">{{pageData.config.x}}</text> {{t('newAct.r_r8')}} <text class="error">{{pageData.config.n}}</text> {{currency}}.
					</view>
				</view>
