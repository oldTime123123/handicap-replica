<template>
	<view class="normalBg" style="background: #060D1F">
		...
		<view class="pdlr30 mt50">
				...
				<view style="width:100% ; " class="mt29 flex col_center pdlr" v-if="!isPre">
					<view style="width:400rpx">
						<nut-progress style="height: 28rpx;"
							...
							:is-show-percentage="true" :text-inside="false" :text-color="'#53FCFF'"
							:stroke-color="'#53FCFF'" />
					</view>
					<view class="f20 mglr23">
						{{(pageData.sale_panel>pageData.total_panel) ? pageData.total_panel:pageData.sale_panel }}/{{pageData.total_panel}}
					</view>
				</view>
			...
			<!-- Coupon -->
			<view class="mt30">
				<view class="text_bold">{{t('parWel.c_l10')}}</view>
				<view class="between" style="flex-wrap: wrap;word-break: normal;">
					<view class="couponItem between mt30" v-for="(item,index) in myCouponList" :key="index"
						...
						<view class="pl30">
							<view class="">{{t('parWel.c_l3')}} <text style="color: #666;">{{item.price}}:
									{{currency}}</text>
							</view>
						...
						<view class="pr30" style="width:60rpx;height: 60rpx;position: absolute;right:0;bottom:-10rpx;"
							size="30">
							<IconFont name="Check" v-if="item.isUsed" color="#666"></IconFont>
						</view>
					</view>
				</view>
			</view>
			...
			<view class="botBtn">
				...
				<view class="btns paid text_white"
					:style="youhuiNum ==0?{background:'#53FCFF',color:'#000'}:{background:'#ccc'}"
					@click="showMaskHandle">
					{{t('construct.c_c15')}}

					<text style="color:#000">{{youhuiNum}} {{currency}}</text>
				</view>
			</view>
				...
				<div class="content ">
					...
					<view class="between">
						<view style="border: 1rpx solid;"
							:style="{'border-color':'#53FCFF',color:'#000'}"
							@click="loginOutMask = false">{{t('all.a_c1')}}
						</view>
						<view class="text_white" :style="{background:'#53FCFF',color:'#000'}" @click="confirmHandle">
							{{t('all.a_c2')}}
						</view>
					</view>
				</div>
