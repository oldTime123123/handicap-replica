	...
	<view class="container cont1">
		<view class="heads">
			...
			<view class="right-icon ml40" @click="viewOpen = !viewOpen" v-show="false">
				<image v-if="viewOpen" src="/src/static/view-open.png" mode="aspectFit"
					style="width: 72upx; height: 72upx">
				</image>
				<image v-else src="/src/static/view-off.png" mode="aspectFit" style="width: 72upx; height: 72upx">
				</image>
			</view>
		</view>
		<!-- k线图 / 表格 -->
		...
		<view class="btn-normal submit-btn h88" style="text-align: center" @click="buyTostShow = true">
			{{ t('pk.t_p2') }}
		...
		<scroll-view scroll-x="true">
			<view class="sec-scroll-view">
				<view class="sec-item" :class="{ 'active-sec': times == item.time }"
					v-for="(item, index) in pDetial.times" @click="times = item.time">
	...
	<view class="buyTost" v-show="buyTostShow">
		<view class="buyCard">
			<view class="flex ml32 mt48 mr32" style="text-align: center;justify-content: center; position: relative;">
				...
				<img style="position: absolute; top: 0; right: 0;" src="../../static/themeNum1/l_icon/down.png" alt=""
					class="ml176" @click="buyTost(false)">
			</view>
			...
			<!-- {{ amount + (amount * rate) }} -->
			<view class="mt16 paddingLR30" style="font-weight: 400;font-size: 32rpx;color: #04E1F4;">
				{{ t('pk.t_p10') }}:
				1%~10%
			</view>
			<view class="mt16 paddingLR30" style="font-weight: 400;font-size: 32rpx;color: #04E1F4;">
				{{ t('pk.t_p11') }}:
				{{
               (amount10 * 0.01).toFixed(2) }}{{currency}} ~ {{ (amount10 * 0.1).toFixed(2) }}{{currency}}
			</view>
